...
namespace MedianSelectionBasicOp
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int size = 5000; size <= 20000; size += 500) {
                int totalBasicOpCount = 0;
                double avgBasicOpCount = 0;

                //do numTests tests for each input size
                for (int i = 1; i <= numTests; i++) {
                    basicOpCounter = 0;

                    //generate array and run algorithms
                    int[] A = GenerateArray(size);
                    Median(A);

                    //increase total basic operations
                    totalBasicOpCount += basicOpCounter;
                }

                //calculate average basic operations for the current array size
                avgBasicOpCount = totalBasicOpCount / numTests;
                Console.WriteLine(avgBasicOpCount);
            }
        ...
        static int Median(int[] A) {
            if (A.Length == 1) {
                ...
                return Select(A, 0, Convert.ToInt32(Math.Floor(A.Length / 2.0)), A.Length - 1); // NB: The third argument is rounded down
            }
        }
